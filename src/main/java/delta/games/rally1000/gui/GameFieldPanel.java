...
public class GameFieldPanel extends JPanel
{
  ...
  private static final int NB_MAX_CARDS_PER_STACK=NB_MAX_KM_CARDS/NB_KM_STACKS+(((NB_MAX_KM_CARDS%NB_KM_STACKS)!=0)?1:0);
  ...
  public void paintComponent(Graphics g)
  {
    ...
    ImagesManager iMgr=ImagesManager.getInstance();
    // Draw safeties
    {
      int x=X_SAFETIES;
      Card[] safeties=_shownCards.getExposedSafeties();
      Boolean[] coupFourres=_shownCards.getCoupFourres();
      int size=Math.min(safeties.length,coupFourres.length);
      for(int i=0;i<size;i++)
      {
        String cardName=safeties[i].getModel().getName();
        if (coupFourres[i].booleanValue())
        {
          cardName="coupFourre_"+cardName;
        }
        Image image=iMgr.getImage(cardName);
        if (coupFourres[i].booleanValue())
        {
    ...
    if (speedLimit!=null)
    {
      Image i=iMgr.getImage(speedLimit.getModel().getName());
    ...
    if (attackCard!=null)
    {
      Image i=iMgr.getImage(attackCard.getModel().getName());
    ...
    // Draw kilometers
    {
      ...
      for(int i=0;i<cards.length;i++)
      {
        ...
        Image image=iMgr.getImage(model.getName());
